...
// lets evaulate what is happening here. 

// 1) First it checks if the value is an empty string. If it is, then the prop is deleted.
// 2) Then, if prop is not "tracks" and the value is not an empty string. The prop is set to the value.
// 3) If that check doesn’t pass, it next checks if prop is equal to tracks, the value is not an empty string, and the record does not have a tracks array. 
// The "tracks" array is initialized with the only contents being value.
// 4) It next checks if prop is equal to tracks, the value is not an empty string. The "tracks" array must exist because the case above was not true. The value is pushed onto the end of the "tracks" array.
